...
enum states4{start4, init, On2, Off2} state4;
unsigned char voice = 0x00;
void speaker(){
	switch(state4){
		case start4:
			state4 = init;
...
enum state5{start5, noiseon, noiseoff} state5;
void combine(){
	if(blink == 0xFF){
		PORTB  = three | 0x08;
	}
...
int main(void) {
	...
	volatile unsigned long combinetogether = 10;
	volatile unsigned long speakernoise = 2;
	PORTB = 0x00;
	TimerSet(1);
	TimerOn();	
    while (1) {
